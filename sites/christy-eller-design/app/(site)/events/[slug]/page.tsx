...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const when = next
    ? formatEventRange(next.startsAt, next.endsAt, ev.timezone)
    : formatEventRange(ev.startsAt, ev.endsAt, ev.timezone)
  return {
    title: ev.title,
...
export default async function EventPage({ params }: Params) {
  ...
  if (!ev) notFound()
  const cover =
    ev.cover && ev.cover.width && ev.cover.height
      ? { key: ev.cover.key, width: ev.cover.width, height: ev.cover.height, alt: ev.cover.alt }
      : null
  const next = nextOccurrence(ev)
  ...
  return (
    <>
      ...
      <Section className="!pt-0">
        <Container narrow>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {next && (
              <ButtonLink
                href={`/events/${ev.slug}/calendar${ev.recurrence ? `?at=${encodeURIComponent(next.startsAt.toISOString())}` : ''}`}
                variant="outline"
              >
                Add to calendar
              </ButtonLink>
      ...
      {ev.registration && next && (
        <ContactForm
          variant="register"
          title="Sign up"
          tone="surface"
          event={{ id: ev.id, title: ev.title, date: formatEventDate(next.startsAt, ev.timezone) }}
        />
      )}
      <JsonLd
        data={{
