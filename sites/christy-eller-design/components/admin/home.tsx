...
export function Home(): ReactNode {
  ...
  const rank = (c: CollectionMeta) =>
    c.inbox ? 0 : c.singleton ? 4 : !c.readOnly && c.view !== 'grid' ? 1 : c.view === 'grid' ? 2 : 3
...
function HomeCard(p: { meta: CollectionMeta; page: Page | undefined }): ReactNode {
  ...
  const action = meta.singleton
    ? { label: 'Edit', to: [meta.name] }
    : meta.view === 'grid'
      ? { label: `Add ${meta.label.toLowerCase()}`, to: [meta.name] }
      : !meta.readOnly
        ? { label: `New ${meta.labelSingular.toLowerCase()}`, to: [meta.name, 'new'] }
        : null
        ...
        {!page ? (
          ...
          <div className="grid cursor-pointer grid-cols-6 gap-1.5" onClick={() => go([meta.name])}>
            {rows.filter(isImageRow).map((r) => (
              ...
              <img
                key={String(r['id'])}
                src={mediaUrl(mediaBaseUrl, String(r['key']))}
                alt=""
                loading="lazy"
          ...
          <ul className="divide-y">
            {rows.map((r) => (
              <li
                key={String(r['id'])}
                className={cn(
                  'flex cursor-pointer items-center gap-3 py-2 text-sm hover:text-primary',
                  meta.inbox && !r['readAt'] && 'font-semibold',
                )}
                onClick={() => open(r)}
              >
                <span className="min-w-0 flex-1 truncate">
                  {titleOf(r, meta)}
                  {meta.inbox && previewOf(r) && (
                    <span className="font-normal text-muted-foreground"> — {previewOf(r)}</span>
                  )}
                </span>
                {meta.publishable && typeof r['status'] === 'string' && <StatusPill meta={meta} row={r} />}
                {money && typeof r[money] === 'number' && (
                  <span>{formatCell(meta.fields[money], money, r[money], 90, r)}</span>
                )}
                <span className="shrink-0 text-xs text-muted-foreground">
                  {fmtDate(r[meta.dateField], { time: false })}
                </span>
              </li>
            ))}
          </ul>
