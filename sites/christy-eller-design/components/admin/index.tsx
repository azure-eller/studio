...
function Shell(p: AdminProps & { api: AdminContext['api']; email: string; onSignOut: () => Promise<void> }): ReactNode {
  ...
  if (!meta) view = <Home />
    ...
    view = meta.singleton ? (
      <Singleton key={meta.name} meta={meta} />
    ) : meta.view === 'grid' ? (
      <Grid key={meta.name} meta={meta} />
    ) : (
      <Table key={meta.name} meta={meta} />
    )
    ...
    <Ctx.Provider value={ctx}>
      <div className="grid min-h-dvh md:grid-cols-[230px_1fr]">
        ...
        <div className="min-w-0">
          <div
            data-admin="top"
            className="sticky top-0 z-30 flex items-center justify-between border-b bg-background px-4 py-2.5 md:hidden"
          >
    ...
    <div className="flex min-h-dvh items-center justify-center p-4">
      <form onSubmit={submit} className="w-[360px] max-w-full rounded-xl border bg-card p-7 shadow-sm">
        ...
        {params?.get('error') === 'invalid_link' && (
          <Note kind="err">That link is invalid or expired. Request a new one.</Note>
        )}
        {state === 'sent' ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            />
            {state === 'rate_limited' && (
              <Note kind="err">Too many sign-in attempts. Wait a few minutes and try once more.</Note>
            )}
            {state === 'error' && (
              <Note kind="err">
                Something went wrong sending the link. Try again; if it keeps happening, tell your site administrator.
              </Note>
            )}
  ...
  return (
    <div
      className={cn(
        ...
        p.kind === 'ok' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700',
      )}
    >
      {p.children}
    </div>
