...
const image = z.object({
  key: z
    ...
    .max(300)
    .describe(
      "R2 object key under this site's prefix, or /photos/<file> for an image committed to this repo's public/ folder.",
    ),
...
export const briefSchema = z
  .object({
    ...
    slug: z
      ...
      .describe('Reserved at intake step 1. Repo, Neon project, Vercel project, R2 prefix and studio subdomain.'),
    org: z.object({
    ...
    timezone: z
      ...
      .describe('IANA zone, e.g. America/Denver.'),
    direction: z
      .string()
      .regex(/^[a-z0-9-]+$/)
      .describe('A folder under design/directions/, chosen by the client from thumbnails.'),
        ...
        posts: z
          .array(
            z.object({
              ...
              coverPhotoKey: z.string().max(300).optional(),
            }),
          )
          .max(3)
          .optional(),
        ...
        galleryCollections: z
          .array(
            z.object({
              ...
              photoKeys: z.array(z.string().max(300)).min(1).max(30),
            }),
          )
          .max(5)
          .optional(),
  ...
  .superRefine((b, ctx) => {
    if (!b.pages.includes('home'))
      ctx.addIssue({ code: 'custom', path: ['pages'], message: "pages must include 'home'" })
    const needs: Partial<Record<PageKey, keyof typeof b.features>> = {
      events: 'events',
      posts: 'posts',
      gallery: 'gallery',
      donate: 'donations',
    ...
    for (const p of b.pages) {
      const f = needs[p]
      if (f && !b.features[f])
        ctx.addIssue({ code: 'custom', path: ['pages'], message: `page '${p}' requires features.${f}` })
    }
    if (new Set(b.pages).size !== b.pages.length)
      ctx.addIssue({ code: 'custom', path: ['pages'], message: 'pages must be unique' })
    const keys = new Set(b.media.photos.map((p) => p.key))
    for (const g of b.seed.galleryCollections ?? [])
      for (const k of g.photoKeys)
        if (!keys.has(k))
          ctx.addIssue({ code: 'custom', path: ['seed', 'galleryCollections'], message: `unknown photo key ${k}` })
    for (const p of b.seed.posts ?? [])
      if (p.coverPhotoKey && !keys.has(p.coverPhotoKey))
        ctx.addIssue({ code: 'custom', path: ['seed', 'posts'], message: `unknown photo key ${p.coverPhotoKey}` })
  })
