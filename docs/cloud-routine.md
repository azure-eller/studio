...
| With the routine pointed at the designer's repo, the proxy's API credential came back **401 Bad credentials** (git push of the site branch still worked) | ship no longer uses the pull-request API: it pushes the site branch, then fast-forwards `main` with plain git. Whether the proxy allows a push to `main` is untested as of 2026-09-07 (that run was finished from a laptop); if it refuses, set a real `GH_PAT` (contents: write) in the environment |
