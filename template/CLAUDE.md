...
5. **Core stays behind its boundary.** Import from `@studio/core` only in `lib/collections.ts`, `app/api/site/[...path]/route.ts`, `app/admin/[[...path]]/page.tsx`, and `content`/`RichText` in sections. Never from a subpath. ESLint enforces this.
6. **Do not edit scaffolded files** (`design/active.ts`, `lib/collections.ts`, `app/layout.tsx`, `app/(site)/layout.tsx`, `app/admin/*`, `app/robots.ts`, `app/sitemap.ts`, `app/opengraph-image.tsx`, `components/layout/*`). They are regenerated from the brief.
