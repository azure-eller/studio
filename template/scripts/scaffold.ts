...
const calendarRoute = `// Generated by \`pnpm scaffold\` — do not edit. "Add to calendar": one occurrence of an event as an .ics file.
...
export async function GET(req: Request, ctx: { params: Promise<{ slug: string }> }) {
  ...
  const o = at ? nextOccurrence(ev, new Date(at)) : (nextOccurrence(ev) ?? { startsAt: ev.startsAt, endsAt: ev.endsAt })
...
// News and Events exist on every site: the admin always offers them, so their routes must too. A brief that asks for
// the page gets a composed one from the model; otherwise a plain list page waits, out of the nav until there is content.
write('app/(site)/posts/[slug]/page.tsx', postPage, { protect: true })
write('app/(site)/events/[slug]/page.tsx', eventPage, { protect: true })
write('app/(site)/events/[slug]/calendar/route.ts', calendarRoute, { protect: true })
