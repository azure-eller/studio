...
- Never touch: `design/`, `app/globals.css`, `app/layout.tsx`, `app/(site)/layout.tsx`, the `[slug]` routes under `app/(site)`, `app/admin/*`, `components/admin/*`, `components.json`, `components/layout/*`, `lib/collections.ts`, `lib/core.ts`, `app/robots.ts`, `app/sitemap.ts`, `app/opengraph-image.tsx`, anything under `app/api` or `app/admin`, `package.json`.
