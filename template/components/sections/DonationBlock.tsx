...
import { Button, Card, Container, Heading, Input, Label, Section } from '@/components/ui'
  ...
  return (
    <Section labelledBy={id}>
      <Container narrow>
        ...
        <Card className="mt-8">
          {!p.configured ? (
            <p className="text-muted">Online giving isn’t switched on yet. Please get in touch to give another way.</p>
          ) : (
            <form onSubmit={submit}>
              <fieldset>
                ...
                <div className="flex flex-wrap gap-2">
                  {amounts.map((a) => (
                    <label key={a} className={`cursor-pointer rounded-[var(--radius)] border px-4 py-2 text-sm font-semibold ${amount === a && !custom ? 'border-accent bg-accent text-accent-fg' : 'border-line'}`}>
              ...
              <Button type="submit" className="mt-6" disabled={state === 'busy'}>
