...
if [[ "${RUN_BUILD:-}" == "1" ]]; then
  ...
  claude plugin list 2>/dev/null | grep -q "frontend-design@claude-plugins-official" || { claude plugin marketplace add anthropics/claude-plugins-official >/dev/null 2>&1 || true; claude plugin install frontend-design@claude-plugins-official -y; }
  echo "▶ claude /build"
