...
await p.click('button:has-text("Add another") >> nth=1'); await p.fill('input[placeholder="Give"]', 'Donate')
await p.click('button:has-text("Add a quote")')
...
// 7 starting content
await p.click('button:has-text("Add a post")')
...
await p.click('button:has-text("Add an event")')
