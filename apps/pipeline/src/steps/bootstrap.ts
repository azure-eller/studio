...
export async function bootstrap(repoRoot: string): Promise<number> {
  ...
  // 1. Cloudflare zone — discovered, not configured
  ...
  try {
    ...
    const pub = await fetch(`https://${mediaHost}/bootstrap/probe.txt`).catch(() => null)
    await s3.send(new DeleteObjectCommand({ Bucket: e.R2_BUCKET, Key: 'bootstrap/probe.txt' }))
    if (pub?.status === 200) report('r2 bucket + media domain', 'ok', `${e.R2_BUCKET} serves at https://${mediaHost}`)
  ...
  // 4. Resend sending subdomain + DNS + DMARC
  ...
  if (gh(['--version']).status === 0) {
    ...
    const vars: Record<string, string> = { STUDIO_DOMAIN: domain, MEDIA_BASE_URL: `https://${mediaHost}`, DESIGNER_EMAIL: e.DESIGNER_EMAIL, EMAIL_FROM: `Studio <noreply@${sendDomain}>`, NEON_REGION: e.NEON_REGION, CF_ZONE_ID: zoneId, CF_ACCOUNT_ID: accountId, R2_BUCKET: e.R2_BUCKET, MAX_TURNS: '100', ...(process.env['NEON_ORG_ID'] ? { NEON_ORG_ID: process.env['NEON_ORG_ID']! } : {}) }
    let fails = 0
  ...
  try {
    ...
    const existing = await vc.getEnv(p!.id)
    await vc.setEnv(p!.id, {
      STUDIO_DATABASE_URL: pooled, STUDIO_DOMAIN: domain, INTAKE_URL: `https://${intakeHost}`, GH_PAT: e.GH_PAT, GH_ORG: e.GH_ORG, STUDIO_REPO: e.STUDIO_REPO,
    ...
    report('intake app', 'ok', `https://${intakeHost} (env set; deploys on the next git push)`)
  ...
  try {
    ...
    for (const [k, v] of [['STUDIO_DATABASE_URL', pooled], ['STUDIO_DATABASE_URL_UNPOOLED', direct], ['CF_ZONE_ID', zoneId], ['CF_ACCOUNT_ID', accountId], ['MEDIA_BASE_URL', `https://${mediaHost}`], ['INTAKE_URL', `https://${intakeHost}`], ['EMAIL_FROM', `Studio <noreply@${sendDomain}>`], ['TEMPLATE_DIR', path.join(repoRoot, 'template')]] as [string, string][]) {
